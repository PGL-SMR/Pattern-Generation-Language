...
#include <stdio.h>
#include <math.h>
#include "conv_2d_linear/conv_2d_linear_float.h"
#include "conv_2d_linear/conv_2d_linear_double.h"
...
out[(j)+((i)*(((size)-(kernel_size))+(1)))]=0 ;
for(kx=0;kx<kernel_size;++kx) {
for(ky=0;ky<kernel_size;++ky) {
out[(j)+((i)*(size-kernel_size+1))]+=(in[(j+kx+ky)+((i)*(size))])*(kernel[(ky)+((kx)*(kernel_size))]) ;
} 
} 
} 
} 
...
void run_tests_float(){
    float *in = malloc(SIZE*SIZE*sizeof(float));
    float *kernel = malloc(KERNEL_SIZE*KERNEL_SIZE*sizeof(float));
    float *out = malloc(OUT_SIZE*OUT_SIZE*sizeof(float));
    float *out_ref = malloc(OUT_SIZE*OUT_SIZE*sizeof(float));
    ...
    for (int i = 0; i < N_TESTS; i++) {
        ...
        compare_output_float(OUT_SIZE*OUT_SIZE, out, out_ref);
    }
...
void run_tests_double(){
    double *in = malloc(SIZE*SIZE*sizeof(double));
    double *kernel = malloc(KERNEL_SIZE*KERNEL_SIZE*sizeof(double));
    double *out = malloc(OUT_SIZE*OUT_SIZE*sizeof(double));
    double *out_ref = malloc(OUT_SIZE*OUT_SIZE*sizeof(double));
    ...
    for (int i = 0; i < N_TESTS; i++) {
        ...
        compare_output_double(OUT_SIZE*OUT_SIZE, out, out_ref);
    }
